...
## Spring Social流程
![OAuth2.0认证流程示意图](http://img-blog.csdn.net/20180510141511222?watermark/2/text/Ly9ibG9nLmNzZG4ubmV0L1JvYmVydG9IdWFuZw==/font/5a6L5L2T/fontsize/400/fill/I0JBQkFCMA==/dissolve/70/gravity/SouthEast)
在获取到用户信息后构建Authentication放入SecurityContext中完成认证流程。在标准的OAuth2协议中步骤1-6是固定的，只有最后一步不同的服务提供商返回的用户信息是不同的，Spring Social已经为我们封装好了1-6步。(该文章完全参考自[郑龙飞  Spring Security系列](https://github.com/longfeizheng/logback))
...
SpringSocial对应过滤器为SocialAuthenticationFilter
```
### 个性SocialAuthenticationFilter配置
```
...
```
### SocialAuthenticationFilter拦截请求
```
requiresAuthentication()方法判断请求是否需要SocialAuthenticationFilter拦截
SocialAuthenticationFilter拦截请求: /login/qq => {filterProcessesUrl}/{providerId}
其中filterProcessesUrl在个性化SocialAuthenticationFilter时配置，providerId通过请求URI截取
...
private Authentication attemptAuthService(final SocialAuthenticationService<?> authService, final HttpServletRequest request, HttpServletResponse response) throws SocialAuthenticationRedirectException, AuthenticationException {
    ...
    // 2.从SecurityContext获取Authentication判断是否认证
...
```
#### OAuth2AuthenticationService#getAuthToken 获取SocialAuthenticationToken
```
...
```
#### SocialAuthenticationFilter#doAuthentication 对SocialAuthenticationToken进行认证
```
...
```
####  SocialAuthenticationProvider#authenticate SpringSocial认证流程
```
...
```
#### JdbcUsersConnectionRepository#toUserId() 用户关联关系查询
```
